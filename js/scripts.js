function initTrustbook() {
    ...
    // ===== CÓDIGO CORRIGIDO E FINAL PARA O MENU =====

// Mob Menu------------------
// Ação para abrir e fechar o menu no botão
    $(".nav-button-wrap").on("click", function (e) {
        e.stopPropagation(); // Importante para evitar que o clique se propague
        $(".main-header").toggleClass("vismobmenu");
        $(".mob-nav-overlay").fadeToggle(200);
        $(this).toggleClass("nbw_tb");
    });

    // Função genérica para fechar o menu
    function closeMobMenu() {
        $(".main-header").removeClass("vismobmenu");
        $(".mob-nav-overlay").fadeOut(200);
        $(".nav-button-wrap").removeClass("nbw_tb");
    }

    // Ação para fechar o menu ao clicar no overlay (fundo escuro)
    $(".mob-nav-overlay").on("click", function () {
        closeMobMenu();
    });

    // Ação para fechar o menu ao clicar em um link das opções
    $(".main-header .menu-a").on("click", function () {
        // Agora o fechamento é instantâneo, pois não há mais conflito
        closeMobMenu();
    });
